    ...
    public partial class Branch : Form
    {
        ...
        private void Branch_Load(object sender, EventArgs e)
        {
            ...
            while (reader.Read())
            {
                ...
                listViewItem.SubItems.Add(reader["id_branc"].ToString());
                listViewItem.SubItems.Add(reader["branc"].ToString());
        ...
        private void btn_save_Click(object sender, EventArgs e)
        {
            ...
            {
                string query = "INSERT INTO `db_edu` (`id_branc`, `branc`) VALUES (NULL, '" + textBox1.Text + "')";
                MySqlConnection databaseConnection = new MySqlConnection(connectionString);
        ...
        {
            string cmdSql = "SELECT branc FROM db_edu";
            MySqlConnection databaseConnection = new MySqlConnection(connectionString);
        ...
        {
            string cmdSql = "UPDATE db_edu SET branc='" + textBox1.Text + "' WHERE id_branc='" + id.Text + "'";
            MySqlConnection databaseConnection = new MySqlConnection(connectionString);
            MySqlCommand commandDatabase = new MySqlCommand(cmdSql, databaseConnection);
            commandDatabase.Parameters.AddWithValue("branc", textBox1);
            try
            {
                databaseConnection.Open();
                MySqlDataReader myReader = commandDatabase.ExecuteReader();
        ...
        private void btn_del_Click(object sender, EventArgs e)
        {
            ...
            {
                MySqlConnection databaseConnection = new MySqlConnection(connectionString);
                MySqlCommand commandDatabase = new MySqlCommand("DELETE FROM db_edu WHERE id_branc=" + id.Text, databaseConnection);
                try
                {
                    databaseConnection.Open();
